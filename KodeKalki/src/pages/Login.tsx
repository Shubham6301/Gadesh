...
const LightThemeBackground: React.FC = () => {
  const stars = Array.from({ length: 18 }).map((_, i) => {
    ...
    return (
      <span key={i} className="light-falling-star"
        style={{ left: `${left}%`, top: '-2%', animationDelay: `${delay}s`, animationDuration: `${duration}s`, fontSize: `${size}px` }}>
        ★
      </span>
    );
  });
    ...
    <div className="fixed inset-0 w-full h-full overflow-hidden" style={{ pointerEvents: 'none' }}>
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 1920 1080" preserveAspectRatio="none">
        ...
        {Array.from({ length: 30 }, (_, i) => (
          <circle key={i}
            cx={Math.random() * 1920} cy={Math.random() * 1080}
            r={Math.random() * 8 + 2}
            ...
            <animateTransform attributeName="transform" type="translate"
              ...
              dur={`${Math.random()*10+5}s`} repeatCount="indefinite" />
            <animate attributeName="opacity" values="0.05;0.2;0.05"
              dur={`${Math.random()*4+3}s`} repeatCount="indefinite" />
          </circle>
        ))}
        <path d="M0 800 Q480 850 960 800 T1920 800 V1080 H0 Z" fill="#ffffff" opacity="0.1">
          <animate attributeName="d" dur="8s" repeatCount="indefinite"
            values="M0 800 Q480 850 960 800 T1920 800 V1080 H0 Z;M0 820 Q480 780 960 820 T1920 820 V1080 H0 Z;M0 800 Q480 850 960 800 T1920 800 V1080 H0 Z" />
        </path>
      </svg>
    </div>
  );
...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // login() now enforces role-based panel access and redirects automatically
      await login(username, password, panelType);

      // login() does window.location.href redirect internally,
      // but if using React Router navigate instead, use getAndClearRedirectUrl:
        ...
        <div className="relative z-10 w-full max-w-md mx-auto px-6">
          ...
          <div className="flex justify-center mb-6">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-2xl font-bold text-gray-900 dark:text-white">KodeKalki</span>
            </div>
          </div>
          ...
          <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm py-8 px-6 shadow-2xl rounded-xl border border-gray-200 dark:border-gray-700">
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              {/* Error Banner */}
              {error && (
                <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-md p-4 flex items-start space-x-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1 appearance-none block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md placeholder-gray-400 dark:bg-gray-900/80 dark:text-white focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm backdrop-blur-sm"
                  placeholder="Enter your username or email"
                  value={username} onChange={(e) => setUsername(e.target.value)}
                />
              </div>

              {/* ── Panel Selector (User / Admin) ────────────────────────────── */}
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {/* User Button */}
                  <button type="button" onClick={() => setPanelType('user')}
                    className={`flex items-center justify-center px-3 py-2 border rounded-md text-sm font-medium transition-all duration-200 ${
                      panelType === 'user'
                        ...
                        : 'border-gray-300 dark:border-gray-600 bg-white/80 dark:bg-gray-900/80 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800/80'
                    }`}>
                  ...
                  {/* Admin Button */}
                  <button type="button" onClick={() => setPanelType('admin')}
                    className={`flex items-center justify-center px-3 py-2 border rounded-md text-sm font-medium transition-all duration-200 ${
                      panelType === 'admin'
                        ? 'border-orange-500 bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 shadow-md'
                        : 'border-gray-300 dark:border-gray-600 bg-white/80 dark:bg-gray-900/80 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800/80'
                ...
                {/* Dynamic hint text */}
                <p className={`mt-2 text-xs ${panelType === 'admin' ? 'text-orange-600 dark:text-orange-400' : 'text-gray-500 dark:text-gray-400'}`}>
                  {panelType === 'admin'
                    // ? '🔒 Admin accounts only — you will be redirected to the Admin Panel'
                    // : '👤 Regular user accounts — you will be redirected to the User Dashboard'
                    }
                </p>
              </div>
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? (
                  ...
                  `Sign in as ${panelType === 'admin' ? 'Admin' : 'User'}`
                )}
              </button>
